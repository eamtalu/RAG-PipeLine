...
- **SSH fetch (poller AND `POST /logs/fetch-remote`) finalizes automatically.**
  Both go through `fetch_now`, which pulls bytes only. Stitching is NOT triggered here: Stage 1 writes a `log_regroup_pending` ticket in the same transaction as its entries, and the stitch worker (`app/services/workers/log_stitch_worker.py`) drains that queue on its own ~1s loop. A stitch failure is therefore no longer a property of the fetch run - it is recorded on the queue row (`attempts` / `last_error` / `abandoned_at`) and visible via `GET /logs/regroup/status`, which also reports `backing_off_windows` and `next_retry_at`. The ingested rows are already committed and the ticket stays open, so the stitch worker retries; un-stitched `log_regroup_pending` can no longer accumulate silently because something now owns draining it.
