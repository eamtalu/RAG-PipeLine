...
- No argument: use the saved marker in `.claude/whats-changed.state` (a commit SHA). Range is `<marker>..HEAD`.
...
```bash
git rev-parse HEAD > .claude/whats-changed.state
```

Ensure `.claude/whats-changed.state` is in `.gitignore`; add it if missing.
